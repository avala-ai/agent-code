...
### Colors look washed out / no truecolor

```bash
export COLORTERM=truecolor
```

In tmux, also set `default-terminal` and RGB terminal-features. Full guide: [Terminal support](./terminal-support.mdx).

### Clipboard / copy does nothing under tmux
...
Run `/terminal-setup` inside the TUI for the live capability report.

### Esc vs cancel

In the modern TUI, **Esc never cancels a turn** (it clears the draft or dismisses a modal). Use **Ctrl+C** to cancel. See [Keyboard shortcuts](./keyboard-shortcuts.mdx).

## Still Stuck?

- Run `/doctor` for a full environment health check
- Run `/terminal-setup` for TUI/terminal diagnostics
