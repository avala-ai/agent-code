---
title: "Quickstart"
description: "Get up and running with Agent Code in minutes"
---
...
Agent Code works with any major LLM provider. Set a key, or use a subscription login:
...
```bash subscription
agent login codex   # ChatGPT / Codex
agent login xai     # SuperGrok / X Premium
```
</CodeGroup>

See [Authentication](./authentication.mdx) for full options.

## Start the agent

```bash
agent                 # fullscreen interactive TUI
agent -p "say hi"     # headless one-shot
```
